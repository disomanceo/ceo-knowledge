...
import { Brain, CalendarDays, CheckCircle2, Circle, Cpu, LogOut, MessageSquareText, Plus, RefreshCw, Search, ShieldCheck } from 'lucide-react';
import type { DeviceRecord, EventRecord, MemoryRecord, TaskRecord } from '@ceo-knowledge/shared';
import { api } from './api';
import { supabase } from './supabase';

type Tab = 'chat' | 'today' | 'memory' | 'tasks' | 'devices';
...
export default function App(){const[session,setSession]=useState<Session|null>(null);const[loading,setLoading]=useState(true);const[tab,setTab]=useState<Tab>('chat');useEffect(()=>{supabase.auth.getSession().then(({data})=>{setSession(data.session);setLoading(false)});const{data}=supabase.auth.onAuthStateChange((_e,s)=>setSession(s));if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js').catch(()=>{});return()=>data.subscription.unsubscribe()},[]);const tabs=useMemo(()=>[{id:'chat' as Tab,label:'Chat',icon:MessageSquareText},{id:'today' as Tab,label:'Today',icon:CalendarDays},{id:'memory' as Tab,label:'Memory',icon:Brain},{id:'tasks' as Tab,label:'Tasks',icon:CheckCircle2},{id:'devices' as Tab,label:'Devices',icon:Cpu}],[]);if(loading)return <div className="min-h-screen grid place-items-center muted">กำลังเปิด Ceo…</div>;if(!session)return <Login onReady={()=>void supabase.auth.getSession().then(({data})=>setSession(data.session))}/>;const Page=tab==='chat'?ChatPage:tab==='today'?TodayPage:tab==='memory'?MemoryPage:tab==='tasks'?TasksPage:DevicesPage;return <div className="min-h-screen max-w-3xl mx-auto"><header className="h-16 px-4 flex items-center justify-between border-b border-[#202533] sticky top-0 bg-[#090b10]/95 backdrop-blur z-10"><div className="flex items-center gap-2"><div className="w-9 h-9 rounded-xl bg-[#f4c95d] text-black grid place-items-center"><Brain size={19}/></div><div><div className="font-bold leading-4">Ceo</div><div className="text-[11px] muted">Knowledge Cloud</div></div></div><button className="btn px-3" title="ออกจากระบบ" onClick={()=>void supabase.auth.signOut()}><LogOut size={17}/></button></header><main className="p-4 pb-28"><Page/></main><nav className="fixed bottom-0 left-0 right-0 safe-bottom glass z-20"><div className="max-w-3xl mx-auto grid grid-cols-5 px-1 pt-2">{tabs.map(t=>{const I=t.icon,active=tab===t.id;return <button key={t.id} onClick={()=>setTab(t.id)} className={`py-2 flex flex-col items-center gap-1 text-[11px] ${active?'accent':'muted'}`}><I size={20}/><span>{t.label}</span></button>})}</div></nav></div>}
