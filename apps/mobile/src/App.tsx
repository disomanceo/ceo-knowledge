...
function MemoryPage(){
  type MemoryRow=MemoryRecord&{repeat_count?:number;event_at?:string|null;tier?:string;retention_policy?:string;source_kind?:string;lifecycle_status?:string;valid_from?:string|null;valid_to?:string|null;superseded_by?:string|null;canonical_key?:string;source_refs?:string[];reference_path?:string;revision?:number;evidence_status?:string;metadata?:Record<string,any>;replica?:boolean;node_id?:string};
  type MemoryFilter='all'|'important'|'today'|'pinned'|'temporary'|'archived'|'history';
  ...
  const filters:[MemoryFilter,string][]=[['all','Active'],['important','สำคัญ'],['pinned','Pinned'],['temporary','ชั่วคราว'],['history','History'],['archived','Archive'],['today','วันนี้']];
