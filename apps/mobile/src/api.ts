...
export const api = {
  ...
  chat: (message: string, conversationId = '', recentContext: Array<{ role:'user'|'ceo'; text:string; sourceId?:string; query?:string }> = []) => request<any>('/api/chat', { method: 'POST', body: JSON.stringify({ message, conversationId, recentContext:recentContext.slice(-8) }) }),
  memories: (q = '', offset = 0, limit = 30, filter = '') => request<{ memories: Array<MemoryRecord & { repeat_count?: number; event_at?: string|null; tier?: string; retention_policy?: string; source_kind?: string; metadata?:Record<string,unknown> }>; hasMore:boolean; nextOffset:number|null; hiddenQuestionCount:number; consolidatedCount:number }>(`/api/memories?limit=${Math.max(1,Math.min(60,limit))}&offset=${Math.max(0,offset)}${q?`&q=${encodeURIComponent(q)}`:''}${filter?`&filter=${encodeURIComponent(filter)}`:''}`),
