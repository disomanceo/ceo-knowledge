...
export const api = {
  ...
  chat: (message: string, conversationId = '', recentContext: Array<{ role:'user'|'ceo'; text:string; sourceId?:string; query?:string }> = [], router?: {mode?:string;provider?:string;model?:string;backgroundModel?:string}) => request<any>('/api/chat', { method: 'POST', body: JSON.stringify({ message, conversationId, recentContext:recentContext.slice(-8), ...(router?{router}:{}) }) }),
