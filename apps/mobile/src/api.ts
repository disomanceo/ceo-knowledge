...
export const api = {
  ...
  chat: (message: string, conversationId = '', recentContext: Array<{ role:'user'|'ceo'; text:string; sourceId?:string; query?:string; field?:string; resultSet?:Array<{id:string;kind:string;title:string;startAt?:string;dueAt?:string;location?:string}> }> = [], router?: {mode?:string;provider?:string;model?:string;backgroundModel?:string}, clientContext?:{latitude?:number;longitude?:number;timezone?:string}) => request<any>('/api/chat', { method: 'POST', body: JSON.stringify({ message, conversationId, recentContext:recentContext.slice(-8), ...(router?{router}:{}), ...(clientContext?{clientContext}:{}) }) }),
