...
export async function enqueueProviderChat(env:Env, token:string, message:string, searchResults:any[], { idempotencyKey='', provider='auto', strategy='balanced', task='general' }:{idempotencyKey?:string;provider?:string;strategy?:string;task?:string}={}) {
  ...
  const payload={
    device_id:device.id,
    tool:'provider.chat',
    arguments:{message:clean(message,4000),context,provider:clean(provider,40)||'auto',strategy:clean(strategy,40)||'balanced',task:clean(task,40)||'general'},
